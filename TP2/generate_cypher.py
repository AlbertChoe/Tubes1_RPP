...
def generate_cypher(data_file, output_file):
    ...
    # Create Constraints
    cypher_queries.append("CREATE CONSTRAINT IF NOT EXISTS FOR (n:BMWModel) REQUIRE n.name IS UNIQUE;")
    cypher_queries.append("CREATE CONSTRAINT IF NOT EXISTS FOR (n:Series) REQUIRE n.name IS UNIQUE;")
    cypher_queries.append("CREATE CONSTRAINT IF NOT EXISTS FOR (n:BodyType) REQUIRE n.name IS UNIQUE;")
    cypher_queries.append("CREATE CONSTRAINT IF NOT EXISTS FOR (n:Powertrain) REQUIRE n.name IS UNIQUE;")
    cypher_queries.append("CREATE CONSTRAINT IF NOT EXISTS FOR (n:Drivetrain) REQUIRE n.name IS UNIQUE;")
    cypher_queries.append("CREATE CONSTRAINT IF NOT EXISTS FOR (n:Feature) REQUIRE n.name IS UNIQUE;")
    cypher_queries.append("CREATE CONSTRAINT IF NOT EXISTS FOR (n:Performance) REQUIRE n.name IS UNIQUE;")
    ...
    for model in data.get('models', []):
        ...
        query_parts.append(f"WITH m MATCH (s:Series {{name: '{series}'}}) MERGE (m)-[:HAS_SERIES]->(s)")
        query_parts.append(f"WITH m MATCH (b:BodyType {{name: '{body_type}'}}) MERGE (m)-[:HAS_BODY_TYPE]->(b)")
        query_parts.append(f"WITH m MATCH (p:Powertrain {{name: '{powertrain}'}}) MERGE (m)-[:HAS_POWERTRAIN]->(p)")
        query_parts.append(f"WITH m MATCH (d:Drivetrain {{name: '{drivetrain}'}}) MERGE (m)-[:HAS_DRIVETRAIN]->(d)")
        query_parts.append(f"WITH m MATCH (perf:Performance {{name: '{performance}'}}) MERGE (m)-[:HAS_PERFORMANCE]->(perf)")

        for feature in features:
            query_parts.append(f"WITH m MATCH (f:Feature {{name: '{feature}'}}) MERGE (m)-[:HAS_FEATURE]->(f)")
